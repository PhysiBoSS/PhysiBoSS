		...
		if (tokens.size() == 2 && tokens[0].compare("ID") != 0)
		{
			...
			if (cell != NULL)
			{
				try {
					...
					maboss_intra->bnd_filename = tokens[1].substr(0, tokens[1].find_last_of('.')) + ".bnd";
					...
					maboss_intra->indicesOfInputs.clear();
					
				} catch (BNException& e) {
					std::cerr << "Error: Cell with ID " << tokens[0] << " could not load cfg " << tokens[1] << std::endl;
				}
			}
			else {
				std::cerr << "Error: Cell with ID " << tokens[0] << " not found during intracellular_state loading." << std::endl;
...
void MaBoSSIntracellular::load_bndcfgs( std::string input_filename )
{
	std::ifstream file( input_filename, std::ios::in );
	if( !file )
	{ 
	...
	while (std::getline(file, line))
	{
		...
		if (tokens.size() == 3 && tokens[0].compare("ID") != 0)
		{
			PhysiCell::Cell* cell = getById(stoi(tokens[0]));
			if (cell != NULL)
			{
				try {
					MaBoSSIntracellular* maboss_intra = static_cast<MaBoSSIntracellular*>(cell->phenotype.intracellular);
					maboss_intra->bnd_filename = tokens[1];
					maboss_intra->cfg_filename = tokens[2];
					maboss_intra->maboss.init_maboss(maboss_intra->bnd_filename, maboss_intra->cfg_filename);
					maboss_intra->maboss.mutate(maboss_intra->mutations);
					maboss_intra->maboss.set_initial_values(maboss_intra->initial_values);
					maboss_intra->maboss.set_parameters(maboss_intra->parameters);
					...
					maboss_intra->maboss.restart_node_values();
					maboss_intra->indicesOfInputs.clear();
					for (const auto& input: maboss_intra->listOfInputs) {
						maboss_intra->indicesOfInputs.push_back(PhysiCell::find_signal_index(input.second.physicell_name));
					}
					maboss_intra->indicesOfOutputs.clear();
					for (const auto& output: maboss_intra->listOfOutputs) {
						maboss_intra->indicesOfOutputs.push_back(PhysiCell::find_behavior_index(output.second.physicell_name));
					}
