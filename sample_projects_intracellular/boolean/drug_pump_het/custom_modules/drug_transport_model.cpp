...
// {
//     ags_receptor_info.name = "ATP pump model"; 
...
{
	double pumping_rate = ((k2_pump * total_pump_enzyme) * internal_density) / ( Km_pump + internal_density); 
	pumping_rate *= (double) pCell->phenotype.volume.total; // Convert mM/min to amol/min
...
double calculate_diffusion_flux(Cell* pCell, int density_idx, double permeability){
	...
	// Activate pump ONLY if the cell _actually_ has it
	bool pump_state = static_cast<bool>(get_custom_data_variable(pCell, "pump_activated"));
	if (pump_state)
	{
		double k2_pump = get_custom_data_variable(pCell, "k2_pump");
		double Km_pump = get_custom_data_variable(pCell, "Km_pump");
		double total_enzyme_pump = get_custom_data_variable(pCell, "total_enzyme_pump");
		double pump_flux = calculate_atp_pump_flux(pCell, density_int, k2_pump, Km_pump, total_enzyme_pump);
		flux += pump_flux;
	}	
